...
export function AssignModal({ person, assigneeType, onClose }: Props) {
  ...
  const canSubmit = sn.trim().length >= 8 && assignedBy.trim()
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-5 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs text-gray-400 mb-0.5">{assigneeType === 'student' ? 'จับคู่เครื่องให้นักเรียน' : 'จับคู่เครื่องให้ครู'}</p>
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">รองรับสแกน Barcode — กด Enter เพื่อบันทึก</p>
        </div>
